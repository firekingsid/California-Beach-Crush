...
public class AnimationManager : MonoBehaviour
{
	...
	void OnEnable()
	{
		...
		if (name == "MenuPlay")
		{
			...
			{
				transform.Find("Leaderboard").gameObject.SetActive(true);
				Transform container = transform.Find("Leaderboard/Viewport/Content");
				LeaderboardManager.Instance.InitLeaderboardPanel(container, LevelManager.Instance.currentLevel);
			}
			else
			{
				transform.Find("Leaderboard").gameObject.SetActive(false);
			}
			// for (int i = 1; i <= 3; i++)
			// {
		...
		if (name == "MenuComplete")
		{
			...
			if (LeaderboardManager.IsAuthenticated)
			{
				transform.Find("Leaderboard").gameObject.SetActive(true);
				Transform container = transform.Find("Leaderboard/Viewport/Content");
				LeaderboardManager.Instance.PostScore(LevelManager.Instance.currentLevel, LevelManager.Score, container);
			}
			else
			{
				transform.Find("Leaderboard").gameObject.SetActive(false);
...
#endif
#if UNITY_INAPPS
			UnityInAppsIntegration.THIS.BuyProduct(0);
#elif AMAZON
...
#endif
#if UNITY_INAPPS
			UnityInAppsIntegration.THIS.BuyProduct(1);
#elif AMAZON
...
#endif
#if UNITY_INAPPS
			UnityInAppsIntegration.THIS.BuyProduct(2);
#elif AMAZON
...
#endif
#if UNITY_INAPPS
			UnityInAppsIntegration.THIS.BuyProduct(3);
#elif AMAZON
