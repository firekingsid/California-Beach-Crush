...
public class UnityInAppsIntegration : MonoBehaviour, IStoreListener
{
    ...
    public List<string> IAP_Product_ID = new List<string>();
    ...
    public void InitializePurchasing()
    {
        ...
        IAP_Product_ID.Clear();
        for (int i = 0; i < 4; i++)
        {
            IAP_Product_ID.Add(Application.identifier + ".gold" + (i + 1));
            print(IAP_Product_ID[i]);
            //kProductIDConsumableArray[i] = LevelManager.THIS.InAppIDs[i];
            builder.AddProduct(IAP_Product_ID[i], ProductType.Consumable);
        }
    ...
    {
        BuyProductID(IAP_Product_ID[ID]);
    }
    ...
    public PurchaseProcessingResult ProcessPurchase(PurchaseEventArgs args)
    {
        ...
        if (Array.IndexOf(IAP_Product_ID.ToArray(), args.purchasedProduct.definition.id) > -1)
        {
